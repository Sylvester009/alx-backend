...
class LFUCache(BaseCaching):
    ...
    def put(self, key, item):
        ...
        if key in self.cache_data:
            ...
            self.freqs[key] += 1
            self.order.remove(key)  # Update position in order
            # Reinsert to end to mark it as recently used
            ...
            if len(self.cache_data) >= BaseCaching.MAX_ITEMS:
                ...
                """If multiple LFU keys exist,
                use LRU to remove the oldest
                """
                if len(lfu_keys) > 1:
                    for k in self.order:
                        if k in lfu_keys:
            ...
            self.cache_data[key] = item
            self.freqs[key] = 1
            self.order.append(key)  # Track insertion order
    ...
    def get(self, key):
        ...
        self.freqs[key] += 1
        self.order.remove(key)  # Update position in timestamps for LRU
        self.order.append(key)  # Reinsert to end to mark it as recently used
